...
public:
  PreApproach() : Node("pre_approach_node") {
    ...
    auto p3_final_approach_desc = rcl_interfaces::msg::ParameterDescriptor{};
    ...
    p3_final_approach_desc.description =
        "The value of the request of the service, attach_to_shelf(true or "
        "false)";
    ...
    attach_to_shelf_client_ =
        this->create_client<ApproachServiceMessage>("approach_shelf");
  }
  ...
  void timer_callback() {
    ...
    if (robot_stat_ == 1) { // get obstacle distance and go to forward
      ...
      if (t_cnt > 0) {
        ...
        RCLCPP_INFO(this->get_logger(), "Get final_approach param : %s",
    ...
    if (robot_stat_ == 7) {
      while (!attach_to_shelf_client_->wait_for_service(1s)) {
        if (!rclcpp::ok()) {
          RCLCPP_ERROR(
              this->get_logger(),
      ...
      auto request = std::make_shared<attach_shelf::srv::GoToLoading_Request>();
      request->attach_to_shelf = final_approach_;
      service_done_ = false;
      auto result_future = attach_to_shelf_client_->async_send_request(
          request, std::bind(&PreApproach::response_callback, this,
                             std::placeholders::_1));
      RCLCPP_INFO(
          this->get_logger(),
          "Requested '/approach_shelf' service with 'attach_to_shelf' is '%s'",
  ...
  void response_callback(
      rclcpp::Client<ApproachServiceMessage>::SharedFuture future) {
    ...
    if (status == std::future_status::ready) {
      ...
      if (result->complete == true)
        RCLCPP_INFO(this->get_logger(),
                    "Result: success and result.complete is true");
      else
        RCLCPP_INFO(this->get_logger(),
                    "Result: success, but result.complete is false");
...
private:
  ...
  rclcpp::Subscription<sensor_msgs::msg::LaserScan>::SharedPtr
      laser_subscription_;
  rclcpp::Client<ApproachServiceMessage>::SharedPtr attach_to_shelf_client_;
